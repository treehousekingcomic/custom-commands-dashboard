...
@command.route("/server/<int:server_id>/singlerole/edit", methods=["POST", "GET"])
@requires_authorization
def edit_singlerole(server_id):
    ...
    if request.method == "GET":
        template = get_template(
            ...
            server,
            data,
            template_name="app/command/commands",
            variables=variables,
            roles=roles,
        )
    ...
    if request.method == "POST":
        ...
        if cmd and command_id != cmd[0] or cmd2:
            ...
            with psycopg2.connect(**params) as conn:
                with conn.cursor() as db:
                    db.execute(
                        "UPDATE role SET role = %s::bigint[] WHERE command_id = %s",
                        (command_roles, command_id),
                    )
